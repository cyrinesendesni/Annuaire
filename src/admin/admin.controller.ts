...
export class AdminController {
  ...
  async create(@Body() createUserDto: CreateAdminDto , @Res() response, @UploadedFile() file: Express.Multer.File) {
    try {
...
return response.status(HttpStatus.OK).json({
  ...
  async findById(@Param('id') id: string, @Res() response) {
    try {
      const userid= await this.adminService.findUserById(id);
  return response.status(HttpStatus.OK).json({
    data: userid,
    message:"user found successfully",
    status:HttpStatus.OK,
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() updateUserDto: UpdateAdminDto, @Res() response) {
    try {
      const userupdate= await  this.adminService.update(id, updateUserDto);
  ...
  async remove(@Param('id') id: string, @Res() response) {
    try {
      const userdel= await this.adminService.remove(id);
  return response.status(HttpStatus.OK).json({
    data: userdel,
